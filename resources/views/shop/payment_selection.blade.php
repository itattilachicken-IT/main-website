...
<div class="container my-4">
    <div class="payment-card">
        ...
        <div class="payment-summary">
            <p><strong>Total Order:</strong> KES {{ number_format($order->total_amount, 0) }}</p>
            <p><strong>Delivery Fee:</strong> KES {{ number_format($order->delivery_fee, 0) }}</p>

            <p><strong>Payable Now:</strong> KES 
                <span id="payable-now">
            ...
            @if($order->payment_type === 'deposit')
                <p class="text-warning"><strong>Remaining Balance:</strong> KES 
                    <span id="remaining-balance">
                        {{ number_format($order->total_amount - $order->paid_amount, 0) }}
                    </span>
                </p>
                <p class="small text-muted">The remaining balance can be paid on or before delivery using the link sent to your email.</p>
            @else
                <p class="text-muted">No remaining balance.</p>
            @endif
        </div>
        ...
        <form action="{{ route('checkout.process_payment', $order->id) }}" method="POST">
            ...
            <div class="payment-grid">
                ...
                <label class="payment-option">
                    ...
                    <img src="{{ asset('images/logos/visa.png') }}" class="payment-logo"> <span class="payment-label">Visa / Mastercard</span>
                </label>
                <label class="payment-option">
                ...
                <label class="payment-option">
                    <input type="radio" name="payment_method" value="paypal" required>
                    <img src="{{ asset('images/logos/paypal.png') }}" class="payment-logo" alt="PayPal Logo">
