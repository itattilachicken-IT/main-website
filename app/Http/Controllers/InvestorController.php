  ...
  public function store(Request $request)
{
    ...
    try {
        ...
        if (!is_array($placementDates)) $placementDates = [$placementDates];
        if (!is_array($paymentDates)) $paymentDates = [$paymentDates];
        ...
        Mail::raw($message, function ($mail) use ($request) {
            $mail->to($request->email)
                 ->subject('Investor Portal Login Details');
        });

        return redirect()->back()->with(
            'success',
            'Investor created successfully and login details sent.'
        );
        ...
        Log::error('Investor creation failed', [
            'exception' => $exception
        ]);
